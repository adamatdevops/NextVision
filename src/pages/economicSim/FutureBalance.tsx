...
const FutureBalance: React.FC<{ incomes: number[], expenses: number[] }> = ({ incomes, expenses }) => {
    ...
    useEffect(() => {
        const totalIncomes = incomes.reduce((total, income) => total + income, 0) - state.futureGrossIncome - state.futurePartnerGrossIncome;
        const totalExpenses = expenses.reduce((total, expense) => total + expense, 0);
        setIncomeTotal(totalIncomes);
        setExpenseTotal(totalExpenses);
        setBalanceTotal(totalIncomes - totalExpenses);
    }, [incomes, expenses, state.futureGrossIncome, state.futurePartnerGrossIncome]);
